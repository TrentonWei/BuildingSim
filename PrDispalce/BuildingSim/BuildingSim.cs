...
{
    public partial class BuildingSim : Form
    {
        public BuildingSim(IMap cMap, AxMapControl pMapControl)
        {
            InitializeComponent();
            this.pMap = cMap;
        ...
        private void button29_Click(object sender, EventArgs e)
        {
            ...
            #region 计数
            int NodeSCountSum = 0;//小于建筑物节点阈值的建筑物数量
            double AreaSum = 0;//大于建筑物节点阈值的建筑物数量
            #endregion
            ...
            while (pFeature != null)
            {
                IPolygon pPolygon = pFeature.Shape as IPolygon;
                PolygonObject pPolygonObject = PolygonConvert(pPolygon);
                NodeSCountSum = pPolygonObject.PointList.Count + NodeSCountSum;
            ...
            sw.Write("NodeCount:" + NodeSCountSum.ToString()); sw.Write("\r\n");
            sw.Write("Area:" + AreaSum.ToString()); sw.Write("\r\n");
            sw.Close();
            fs.Close();
            #endregion
        }
